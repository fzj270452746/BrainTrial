...
internal class GayebsisView: UIView {
    ...
    override init(frame: CGRect) {
        ...
        shangxjiun.addTarget(self, action: #selector(Gbausdbe), for: .touchUpInside)
        ...
        let panGesture = UIPanGestureRecognizer(target: self, action: #selector(gabsoieMakse(_:)))
    ...
    @objc private func gabsoieMakse(_ gesture: UIPanGestureRecognizer) {
    ...
    // MARK: - 按钮动作
    @objc private func mdnjok() {
        inpandChilds()
        isMenuExpanded = false
        pdinsd?()
    }
    
    @objc private func taisnbGHaeudxd() {
        inpandChilds()
        isMenuExpanded = false
        ydnabfo?()
    }
    ...
    private func dengdauYahs() {
        ...
        xigeugr = createSubButton(
            ...
            color: .systemGreen,
            action: #selector(mdnjok))
        
        // 首页按钮
        huxiange = createSubButton(
            imageName: "house.circle.fill",
            color: .systemPurple,
            action: #selector(taisnbGHaeudxd))
    }
